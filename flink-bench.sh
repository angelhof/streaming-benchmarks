...
KAFKA_VERSION=${KAFKA_VERSION:-"0.8.2.2"}
REDIS_VERSION=${REDIS_VERSION:-"6.0.6"}
...
run() {
  ...
  then
    ...
    echo 'taskmanager.memory.process.size: 2048m' >> $FLINK_DIR/conf/flink-conf.yaml

    #Fetch Spark
    SPARK_FILE="$SPARK_DIR.tgz"
    fetch_untar_file "$SPARK_FILE" "$APACHE_MIRROR/spark/spark-$SPARK_VERSION/$SPARK_FILE"
  elif [ "SETUP_FILIP" = "$OPERATION" ];
  then
    # Specific setup on Filip's laptop
    $MVN clean install
    ...
    #Fetch Storm (Contains ZooKeeper)
    STORM_FILE="$STORM_DIR.tar.gz"
    fetch_untar_file "$STORM_FILE" "$APACHE_MIRROR/storm/$STORM_DIR/$STORM_FILE"
  ...
  then
    JVM_ARGS="-XX:+PrintGC -XX:+PrintGCDetails" "$FLINK_DIR/bin/flink" run ./flink-benchmarks/target/flink-benchmarks-0.1.0.jar --confPath $CONF_FILE &
    sleep 3
